...
// Funkcija za ažuriranje minimalne i maksimalne vrednosti
...
// Na kraju programa ispišite minimalne i maksimalne vrednosti
void printMinAndMax() {
    std::cout << "Minimalna vrednost cweight2: " << min_value << std::endl;
    std::cout << "Maksimalna vrednost cweight2: " << max_value << std::endl;
}
        ...
        for (int j = -iradius; j <= iradius; j++) {
            ...
            if (rx > -1.0 && rx < (double) _IndexSize  &&
                ...
                if (r >= 1 + addSampleStep && r < _height - 1 - addSampleStep && c >= 1 + addSampleStep && c < _width - 1 - addSampleStep) {
                    ...
                    dxx1 = pixels1D[(addSampleR + addSampleStep + 1) * _width + (c + addSampleStep + 1)] + 
																											pixels1D[(addSampleR - addSampleStep) * _width + c] - 
																											pixels1D[(addSampleR - addSampleStep) * _width + (c + addSampleStep + 1)] - 
																											pixels1D[(addSampleR + addSampleStep + 1) * _width + c];
																				dxx2 = pixels1D[(addSampleR + addSampleStep + 1) * _width + (c + 1)] + 
																											pixels1D[(addSampleR - addSampleStep) * _width + (c - addSampleStep)] - 
																											pixels1D[(addSampleR - addSampleStep) * _width + (c + 1)] - 
																											pixels1D[(addSampleR + addSampleStep + 1) * _width + (c - addSampleStep)];

																				dyy1 = pixels1D[(addSampleR + 1) * _width + (c + addSampleStep + 1)] + 
																											pixels1D[(addSampleR - addSampleStep) * _width + (c - addSampleStep)] - 
																											pixels1D[(addSampleR - addSampleStep) * _width + (c + addSampleStep + 1)] - 
																											pixels1D[(addSampleR + 1) * _width + (c - addSampleStep)];

																				dyy2 = pixels1D[(addSampleR + addSampleStep + 1) * _width + (c + addSampleStep + 1)] + 
                ...
                //cout << "dxx2: " << dxx2 << endl;
            	   updateMinAndMax(cweight2);
            }
        }
        // Ispis minimalne i maksimalne vrednosti
    printMinAndMax();
}
